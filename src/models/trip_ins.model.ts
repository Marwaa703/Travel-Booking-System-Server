...
async function createTripInstruction(tripInstruction: TripInstruction): Promise<TripInstruction | null> {
  try {
...
async function getAllInstructionsByTripId(tripId: number): Promise<TripInstruction[]> {
  try {
    const result = await pool.query('SELECT * FROM trip_ins WHERE trip_id = $1;', [tripId]);
    return result.rows;
  } catch (error) {
...
async function getTripInstructionById(id: number): Promise<TripInstruction | null> {
  try {
    const result = await pool.query('SELECT * FROM trip_ins WHERE id = $1;', [id]);
    return result.rows[0];
  } catch (error) {
...
async function updateTripInstruction(id: number, updates: Partial<TripInstruction>): Promise<TripInstruction | null> {
...
export { createTripInstruction, getAllInstructionsByTripId, getTripInstructionById, updateTripInstruction, deleteTripInstruction };
